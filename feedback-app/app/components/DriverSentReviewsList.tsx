...
interface Props {
  initialReviews: DriverSentReview[]
}
...
export default function DriverSentReviewsList({ initialReviews }: Props) {
  ...
  // 2. Filter grouped trips by searching passenger names (recipient)
  const filteredTrips = useMemo(() => {
    ...
    return allGrouped
      .map(trip => {
        ...
        return {
          ...trip,
          matchedRecipient: matchingReview?.recipient?.name || null
        }
      })
      .filter(trip => trip.matchedRecipient !== null)
  ...
  const handleExportAllReviews = () => {
    ...
    const rows = allReviews.map((r: DriverSentReview) => [
      r.id,
      new Date(r.createdAt).toLocaleString("es-AR", { timeZone: "America/Argentina/Buenos_Aires" }),
      r.pool_id,
      r.reservation_id || "",
      r.recipient?.name || r.recipient?.id || "Pasajero",
    ...
    <div className="space-y-6">
      ...
      {paginatedTrips.length > 0 ? (
        <section className="space-y-6">
          {paginatedTrips.map((trip) => {
            ...
            return (
              <Link
                key={trip.poolId}
                href={`/dashboard/driver/sent-reviews/${trip.poolId}`}
                className="block"
              >
                  ...
                  <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {trip.matchedRecipient && (
                            <span className="text-xs bg-[var(--ws-success-soft)] text-[var(--ws-success)] font-black rounded-lg px-2.5 py-1 uppercase tracking-wider">
                              Coincidencia: {highlightText(trip.matchedRecipient, searchQuery)}
                            </span>
                          )}
                        </div>
                      </div>
                      ...
                      <p className="text-[var(--ws-slate)] leading-relaxed">
                        Ver opiniones y feedback enviados a pasajeros.
                      </p>
                    </div>
        ...
        <section className="ws-card ws-card-large">
          ...
          <h2 className="text-3xl font-black tracking-tight mb-3 text-[var(--ws-midnight)]">
            {searchQuery ? "Ningún viaje coincide con tu búsqueda." : "Cuando envíes reseñas, van a aparecer agrupadas acá."}
          </h2>
        </section>
      )}
    </div>
